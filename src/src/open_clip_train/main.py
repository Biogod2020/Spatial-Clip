...
def main(args):
    ...
    if args.use_spatial_dataset:
        ...
        # --- 修改 Dataset 和 Collate 的实例化 ---
        dataset = SpatiallyAwareDataset(
            Path(args.spatial_data_dir), 
            k_neighbors=args.k_neighbors,
            preprocess_fn=preprocess_train,  # <-- 传给 Dataset
            tokenizer=tokenizer             # <-- 传给 Dataset
        )
        spatial_collate = SpatiallyAwareCollate() # <-- 不再需要参数
